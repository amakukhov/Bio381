### ggplot2 package R presentation Notes- Alex
### Wed, May 3rd, 2017
### ADG

# Structure of ggplot: it's additive, adds on layers (everything added with +) 
  plot_name <- ggplot(data, aes(xVar=x, yVar=y)) + geom_blank
  
  plot_name + func1() + func2()
...
# Clear memory of variables and objects:
rm(list=ls())
...
plot1 + theme_minimal(base_size = 17) + geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.2, position = position_dodge(0.9)) + coord_cartesian(ylim=c(0.40))
...
plot4 + geom_boxplot() + theme_minimal(base_size = 17) # changing base size makes plot easier to read (?)
...
plot7 + geom_point(aes(color = Mass)) + theme_minimal(base_size = 17) + geom_smooth(method = "lm", se = TRUE) # se=FALSE will get rid of standard error
...
# RColorBrewer works well with ggplot
