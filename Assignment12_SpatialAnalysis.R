...
# Because of errors, needed to do the following:
remove.packages(c("ggplot2","ggmap"))
install.packages("ggplot2", type = "source")
install.packages("ggmap", type = "source")

map + geom_point(data = points, aes(x = Longitude, y = Latitude), color="red", size=3, alpha=0.5) # alpha makes it transparent


### Making another, but simpler, map
library(maps)
...
# bioclim <- getData('worldclim', var='bio', res=10) # should be able to download the bioclim data this way in the future, for now it looks like their website is down.
...
bioclimLC1<-crop(x=bioclimLC, extent(-74,-71, 42.5, 45.5)) # cropping it to Vermont's extent (xmin, xmax, ymin, ymax)
plot(bioclimLC1)

### Giving data fake presence and absence
set.seed(44) # have to resubset sites - these analyses want numbers only # can use set.seed to make results significant - Lauren used a for loop from values 1-100 and found that a set.seed value of 44 gave a significant p-value.
...
# plotting the GLM
plot(x=dbio1$bio1,y=fakeP,xlab="Annual Mean Temperature (C)",ylab="P(Disease Presence)", type='n', ylim=c(0,1))
...
library(rworldmap)
# quartz() makes another window where you can have a plot made in
...
presPoints<-data.frame(lon=PresPoints[,2],lat=PresPoints[,1]) # need to switch order for Maxent to have longitude listed first
